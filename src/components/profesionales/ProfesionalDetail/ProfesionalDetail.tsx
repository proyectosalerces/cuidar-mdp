'use client';

/**
 * ProfesionalDetail — Full-page detail view for a single healthcare professional.
 *
 * Displays breadcrumbs, profile header with avatar, two-column layout
 * with description/obras sociales on the left and contact/schedule/address
 * on the right in a sticky sidebar.
 */

import { useState } from 'react';
import Link from 'next/link';
import { cn } from '@/utils/classnames';
import { Badge, Rating, Button } from '@/components/ui';
import { ESPECIALIDAD_LABELS } from '@/utils/constants';
import { formatTelefono, buildWhatsAppLink } from '@/utils/formatters';
import type { Profesional } from '@/types/profesional';
import ResenaSection from '@/components/resenas/ResenaSection/ResenaSection';
import AuthModal from '@/components/auth/AuthModal/AuthModal';
import styles from './ProfesionalDetail.module.css';

/* ── Helpers ──────────────────────────────────────────────────────────── */

function getInitials(name: string): string {
  const cleaned = name.replace(/^(Dra?\.?|Lic\.?)\s*/i, '').trim();
  const parts = cleaned.split(/\s+/);
  const first = parts[0]?.[0] ?? '';
  const last = parts.length > 1 ? parts[parts.length - 1]![0] : '';
  return (first + last).toUpperCase();
}

/* ── Inline SVG Icons ─────────────────────────────────────────────────── */

function PhoneIcon() {
  return (
    <svg className={styles.contactIcon} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
      <path d="M22 16.92v3a2 2 0 01-2.18 2 19.79 19.79 0 01-8.63-3.07 19.5 19.5 0 01-6-6 19.79 19.79 0 01-3.07-8.67A2 2 0 014.11 2h3a2 2 0 012 1.72 12.84 12.84 0 00.7 2.81 2 2 0 01-.45 2.11L8.09 9.91a16 16 0 006 6l1.27-1.27a2 2 0 012.11-.45 12.84 12.84 0 002.81.7A2 2 0 0122 16.92z" />
    </svg>
  );
}

function EmailIcon() {
  return (
    <svg className={styles.contactIcon} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
      <path d="M4 4h16c1.1 0 2 .9 2 2v12c0 1.1-.9 2-2 2H4c-1.1 0-2-.9-2-2V6c0-1.1.9-2 2-2z" />
      <polyline points="22,6 12,13 2,6" />
    </svg>
  );
}

function WhatsAppIcon() {
  return (
    <svg width="20" height="20" viewBox="0 0 24 24" fill="currentColor">
      <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z" />
    </svg>
  );
}

function ClockIcon() {
  return (
    <svg className={styles.infoIcon} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
      <circle cx="12" cy="12" r="10" />
      <polyline points="12 6 12 12 16 14" />
    </svg>
  );
}

function MapPinIcon() {
  return (
    <svg className={styles.infoIcon} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
      <path d="M21 10c0 7-9 13-9 13s-9-6-9-13a9 9 0 0118 0z" />
      <circle cx="12" cy="10" r="3" />
    </svg>
  );
}

function CalendarIcon() {
  return (
    <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
      <rect x="3" y="4" width="18" height="18" rx="2" ry="2" />
      <line x1="16" y1="2" x2="16" y2="6" />
      <line x1="8" y1="2" x2="8" y2="6" />
      <line x1="3" y1="10" x2="21" y2="10" />
    </svg>
  );
}

/* ── Props ─────────────────────────────────────────────────────────────── */

interface ProfesionalDetailProps {
  profesional: Profesional;
}

/* ── Component ────────────────────────────────────────────────────────── */

export default function ProfesionalDetail({ profesional }: ProfesionalDetailProps) {
  const [showAuthModal, setShowAuthModal] = useState(false);

  const whatsAppLink = buildWhatsAppLink(
    profesional.telefono,
    `Hola, me contacto desde Cuidar MdP por una consulta con ${profesional.nombre}.`,
  );

  return (
    <>
      {/* Breadcrumbs */}
      <nav className={styles.breadcrumbs} aria-label="Migas de pan">
        <Link href="/">Inicio</Link>
        <span className={styles.separator} aria-hidden="true">›</span>
        <Link href="/profesionales">Profesionales</Link>
        <span className={styles.separator} aria-hidden="true">›</span>
        <span className={styles.current}>{profesional.nombre}</span>
      </nav>

      {/* Profile Header */}
      <header className={styles.profileHeader}>
        <div className={styles.avatarLarge}>
          <span className={styles.initialsLarge}>
            {getInitials(profesional.nombre)}
          </span>
        </div>

        <div className={styles.profileInfo}>
          <h1 className={styles.profileName}>{profesional.nombre}</h1>
          <div className={styles.profileMeta}>
            <Badge variant="primary">
              {ESPECIALIDAD_LABELS[profesional.especialidad]}
            </Badge>
            <Rating
              value={profesional.calificacion}
              count={profesional.cantidadResenas}
            />
          </div>
          {profesional.matricula && (
            <p className={styles.matricula}>
              Matrícula: {profesional.matricula}
            </p>
          )}
        </div>
      </header>

      {/* Two-Column Layout */}
      <div className={styles.twoColumn}>
        {/* Left — Main Content */}
        <main className={styles.mainContent}>
          <section className={styles.section}>
            <h2 className={styles.sectionTitle}>Sobre el profesional</h2>
            <p className={styles.sectionText}>{profesional.descripcion}</p>
          </section>

          <section className={styles.section}>
            <h2 className={styles.sectionTitle}>Especialidad</h2>
            <p className={styles.sectionText}>
              {ESPECIALIDAD_LABELS[profesional.especialidad]}
            </p>
          </section>

          {profesional.obrasSociales && profesional.obrasSociales.length > 0 && (
            <section className={styles.section}>
              <h2 className={styles.sectionTitle}>Obras sociales</h2>
              <div className={styles.obrasSocialesList}>
                {profesional.obrasSociales.map((os) => (
                  <Badge key={os} variant="outline">
                    {os}
                  </Badge>
                ))}
              </div>
            </section>
          )}

          {/* Reviews */}
          <ResenaSection
            entidadId={profesional.id}
            entidadTipo="profesional"
            entidadNombre={profesional.nombre}
            onRequestAuth={() => setShowAuthModal(true)}
          />
        </main>

        {/* Right — Sticky Sidebar */}
        <aside className={styles.sidebar}>
          {/* Contact Card */}
          <div className={styles.contactCard}>
            <h3 className={styles.cardTitle}>Contacto</h3>

            <div className={styles.contactItem}>
              <PhoneIcon />
              <a href={`tel:${profesional.telefono.replace(/\D/g, '')}`}>
                {formatTelefono(profesional.telefono)}
              </a>
            </div>

            {profesional.email && (
              <div className={styles.contactItem}>
                <EmailIcon />
                <a href={`mailto:${profesional.email}`}>
                  {profesional.email}
                </a>
              </div>
            )}

            <Button
              variant="whatsapp"
              fullWidth
              href={whatsAppLink}
              icon={<WhatsAppIcon />}
              className={styles.whatsappButton}
            >
              Enviar WhatsApp
            </Button>
          </div>

          {/* Schedule */}
          {profesional.horarioAtencion && (
            <div className={styles.infoCard}>
              <h3 className={styles.cardTitle}>Horario de atención</h3>
              <p className={styles.infoValue}>
                <ClockIcon />
                {profesional.horarioAtencion}
              </p>
            </div>
          )}

          {/* Address */}
          <div className={styles.infoCard}>
            <h3 className={styles.cardTitle}>Consultorio</h3>
            <p className={styles.infoValue}>
              <MapPinIcon />
              {profesional.direccionConsultorio}
            </p>
            <p className={styles.infoValue}>
              {profesional.barrio}, Mar del Plata
            </p>
          </div>

          {/* CTA */}
          <Button variant="primary" fullWidth size="lg" icon={<CalendarIcon />}>
            Solicitar turno
          </Button>
        </aside>
      </div>

      {/* Auth Modal */}
      <AuthModal
        isOpen={showAuthModal}
        onClose={() => setShowAuthModal(false)}
      />
    </>
  );
}
